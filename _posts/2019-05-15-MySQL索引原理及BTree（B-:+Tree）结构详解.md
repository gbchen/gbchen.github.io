---
layout:     post
title:      MySQL索引原理及BTree（B-/+Tree）结构详解
subtitle:   WHY要用索引？
date:       2019-05-15
author:     CHEN
...
> MySQL数据库支持多种索引类型，如BTree索引，哈希索引，全文索引等等。本文只关注于BTree索引，这是平常使用MySQL时主要打交道的索引。
...
![](https://ws1.sinaimg.cn/large/006tNc79ly1g31t0e1bgzj30zy0csq43.jpg)
...
![](https://ws3.sinaimg.cn/large/006tNc79ly1g31t2aly0jj312m0d8q45.jpg)
...
![](https://ws4.sinaimg.cn/large/006tNc79ly1g31t78b17ej30p40fkjsi.jpg)
...
![](https://ws2.sinaimg.cn/large/006tNc79ly1g31t9eju03j30h809smxh.jpg)

一个磁盘由大小相同且同轴的圆形盘片组成，磁盘可以转动（各个磁盘必须同步转动）。在磁盘的一侧有磁头支架，磁头支架固定了一组磁头，每个磁头负责存取一个磁盘的内容。磁头不能转动，但是可以沿磁盘半径方向运动（实际是斜切向运动），每个磁头同一时刻也必须是同轴的，即从正上方向下看，所有磁头任何时候都是重叠的（不过目前已经有多磁头独立技术，可不受此限制）。

图7是磁盘结构的示意图。

![](https://ws3.sinaimg.cn/large/006tNc79ly1g31tag8f67j30hw0g4t9i.jpg)
...
![](https://ws4.sinaimg.cn/large/006tNc79ly1g31tkxakryj31230u0djg.jpg)

这里设表一共有三列，假设我们以Col1为主键，则图8是一个MyISAM表的主索引（Primary key）示意。可以看出MyISAM的索引文件仅仅保存数据记录的地址。在MyISAM中，主索引和辅助索引（Secondary key）在结构上没有任何区别，只是主索引要求key是唯一的，而辅助索引的key可以重复。如果我们在Col2上建立一个辅助索引，则此索引的结构如下图所示：

![](https://ws1.sinaimg.cn/large/006tNc79ly1g31tol59l8j311r0u0wi7.jpg)
...
![](/Users/chen/Library/Application Support/typora-user-images/image-20190515114124667.png)

图10是InnoDB主索引（同时也是数据文件）的示意图，可以看到叶节点包含了完整的数据记录。这种索引叫做聚集索引。因为InnoDB的数据文件本身要按主键聚集，所以InnoDB要求表必须有主键（MyISAM可以没有），如果没有显式指定，则MySQL系统会自动选择一个可以唯一标识数据记录的列作为主键，如果不存在这种列，则MySQL自动为InnoDB表生成一个隐含字段作为主键，这个字段长度为6个字节，类型为长整形。

第二个与MyISAM索引的不同是InnoDB的辅助索引data域存储相应记录主键的值而不是地址。换句话说，**InnoDB的所有辅助索引都引用主键作为data域**。例如，图11为定义在Col3上的一个辅助索引：

![](/Users/chen/Library/Application Support/typora-user-images/image-20190515114250387.png)
...
此时索引使用情况和情况二相同，因为中间未提供，所以查询只用到了索引的第一列。

解决方法：

- 可以新建组合索引。
- 强行增加中间的条件，利用in语句或者其他的条件，使之成为情况一
...
范围列可以用到索引（必须是最左前缀），但是范围列后面的列无法用到索引。同时，索引最多用于一个范围列，因此如果查询条件中有两个范围列则无法全用到索引。特别要说明MySQL一个有意思的地方，那就是仅用explain可能无法区分范围索引和多值匹配，因为在type中这两者都显示为range。同时，用了“between”并不意味着就是范围查询。？？？
...
选择性还不错，但离0.9313还是有点距离，那么把last_name前缀加到4：选择性就增加到了0.09007，就这样依次试探。
...
![img](https://ws3.sinaimg.cn/large/006tNc79ly1g3255eed1cj30s00bcaaq.jpg)
...
![img](https://ws1.sinaimg.cn/large/006tNc79ly1g3255xvyp9j30fc0ba3yw.jpg)
